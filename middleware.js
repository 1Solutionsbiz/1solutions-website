...
export function middleware(request) {
  // The actual bug (confirmed by directly inspecting request headers):
  // Next sets x-nextjs-data on the JSON requests its own client router
  // fires for every in-app navigation/prefetch, and for those requests
  // nextUrl.pathname is normalized to the page's own path (no trailing
  // slash) rather than the literal /_next/data/<buildId>/*.json URL - so
  // it matched this same SLASH_PAGES redirect and came back as an
  // x-nextjs-redirect header instead of a normal Location. The client
  // router doesn't expect a redirect signal on a route it's already
  // navigating to, which corrupted the transition (page rendering
  // half-way with dead event handlers) - while every hard/direct page
  // load stayed unaffected, which is why curl against the real page URL
  // alone never showed a problem. A matcher-level exclusion for
  // /_next/data doesn't work here: Next's own compiled matcher regex
  // auto-strips the /_next/data/<buildId> prefix before testing it
  // against your pattern (visible in .next/server/middleware-manifest.json),
  // so the exclusion has to happen inside the function body instead.
