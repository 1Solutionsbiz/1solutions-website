 ...
 * Used to calculate total pages for numbered pagination.
 */
export async function getTotalPostCount() {
  ...
  try {
    ...
    // WPGraphQL with WP GraphQL Smart Cache exposes pageInfo.total
    if (data?.posts?.pageInfo?.total) return data.posts.pageInfo.total;
    // Fallback: count slugs (slower but universal)
    const slugQuery = `query { posts(first: 1000, where: { status: PUBLISH }) { nodes { id } } }`;
    const slugData  = await fetchGraphQL(slugQuery);
    return slugData?.posts?.nodes?.length || 0;
  } catch {
    return 0;
  }
}
