    ...
    try {
      ...
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          source: 'Python Development Services', consent: true, recaptchaToken: token,
        }),
      });
    ...
    <>
      <Head>
        ...
        <style>{`
          ...
          /* Orbs */
          .py-aurora { position:absolute; inset:-15%; z-index:0; pointer-events:none; filter:blur(70px) saturate(150%); animation:py-aurora-drift 20s ease-in-out infinite alternate; }
          .py-aurora-b1 { position:absolute; left:20%; top:30%; width:65%; height:65%; border-radius:50%; background:radial-gradient(circle at center,rgba(15,52,96,0.28) 0%,transparent 70%); transform:translate(-50%,-50%); }
          .py-aurora-b2 { position:absolute; left:78%; top:22%; width:48%; height:48%; border-radius:50%; background:radial-gradient(circle at center,rgba(217,119,6,0.20) 0%,transparent 70%); transform:translate(-50%,-50%); }
          .py-aurora-b3 { position:absolute; left:50%; top:82%; width:55%; height:55%; border-radius:50%; background:radial-gradient(circle at center,rgba(26,82,118,0.16) 0%,transparent 70%); transform:translate(-50%,-50%); }
          @keyframes py-aurora-drift { 0%{transform:translate3d(0,0,0) scale(1)} 100%{transform:translate3d(-4%,3%,0) scale(1.10)} }
          ...
          .py-section-title { font-size:48px;font-weight:900;line-height:1.15;letter-spacing:-1px;background:linear-gradient(135deg,#4f46e5,#7c3aed,#a855f7,#ec4899,#3b82f6,#06b6d4,#4f46e5);background-size:300% 300%;animation:aurora-text 6s ease infinite;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:transparent;margin-bottom:10px; }
          ...
          .py-plans-grid { display:grid;grid-template-columns:repeat(4,1fr);gap:20px;align-items:start; }
          .py-pcard { display:flex;flex-direction:column;height:100%;padding:30px 26px;border-radius:20px;border:1.5px solid rgba(15,52,96,0.12);background:rgba(255,255,255,0.80);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);position:relative; }
          .py-pcard--feat { border-color:rgba(217,119,6,0.40);background:linear-gradient(180deg,rgba(254,243,199,0.35) 0%,rgba(255,255,255,0.90) 100%);box-shadow:0 0 60px rgba(217,119,6,0.12),0 8px 40px rgba(15,52,96,0.08); }
          .py-pcard-pop { position:absolute;top:-13px;left:50%;transform:translateX(-50%);padding:4px 14px;border-radius:100px;background:linear-gradient(90deg,#0F3460,#1a5276 50%,#D97706);color:#fff;font-size:10px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;white-space:nowrap;box-shadow:0 4px 16px rgba(217,119,6,0.28); }
          .py-pcard-badge { display:inline-block;font-size:10px;font-weight:700;letter-spacing:0.10em;text-transform:uppercase;color:#6B7280;background:rgba(15,52,96,0.06);padding:4px 12px;border-radius:100px;margin-bottom:14px; }
          .py-pcard-name { display:block;font-size:17px;font-weight:800;color:#0F3460;margin-bottom:10px;line-height:1.3; }
          .py-pcard--feat .py-pcard-name { color:#b45309; }
          .py-pcard-blurb { font-size:13px;color:#4A6080;line-height:1.7;margin:0 0 20px; }
          .py-pcard-features { list-style:none;padding:0;margin:0 0 24px;flex:1; }
          .py-pcard-features li { padding:8px 0;border-top:1px dashed rgba(15,52,96,0.10);font-size:13.5px;color:#374151;display:flex;align-items:flex-start;gap:8px; }
          .py-pcard-features li::before { content:'✓';color:#16a34a;font-weight:700;flex-shrink:0; }
          .py-pcard-cta { display:block;padding:12px 20px;background:rgba(15,52,96,0.07);border:1.5px solid rgba(15,52,96,0.18);border-radius:50px;color:#0F3460;font-size:13.5px;font-weight:700;text-decoration:none;text-align:center;transition:all 0.2s;margin-top:auto; }
          .py-pcard-cta:hover { background:rgba(15,52,96,0.12);transform:translateY(-1px);text-decoration:none; }
          .py-pcard-cta--feat { background:linear-gradient(135deg,#0F3460,#1a5276);border-color:transparent;color:#fff;box-shadow:0 4px 20px rgba(15,52,96,0.28); }
          .py-pcard-cta--feat:hover { background:linear-gradient(135deg,#0a2444,#0F3460);box-shadow:0 6px 28px rgba(15,52,96,0.38);color:#fff; }
          @media(max-width:1024px){ .py-plans-grid { grid-template-columns:repeat(2,1fr); } }
          @media(max-width:600px){ .py-plans-grid { grid-template-columns:1fr; } }
          ...
          .py-faq-heading { font-size:48px;font-weight:900;line-height:1.15;letter-spacing:-1px;background:linear-gradient(135deg,#4f46e5,#7c3aed,#a855f7,#ec4899,#3b82f6,#06b6d4,#4f46e5);background-size:300% 300%;animation:aurora-text 6s ease infinite;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:transparent;margin:0 0 36px; }
      ...
      <div className="py-page">
        <div className="py-aurora" aria-hidden="true">
          <div className="py-aurora-b1" />
          <div className="py-aurora-b2" />
          <div className="py-aurora-b3" />
        </div>

        {/* ── HERO ── */}
        <ServiceHero
          eyebrow="Expert Python Development Company"
          title={<>Python Development Services - <AuroraText>Build Smarter, Scale Faster</AuroraText></>}
          subtext="From custom web applications and REST APIs to machine learning pipelines and automation tools - 1Solutions delivers production-grade Python solutions for businesses across the US, Canada, and Australia."
          primaryCta={{ label: 'Get a Free Consultation Now', href: '#contact' }}
          stats={[
            { label: 'Python Projects', value: '300', suffix: '+' },
            { label: 'Python Experts', value: '30', suffix: '+' },
        ...
        <section className="py-engage-section" id="engagement">
          <div className="py-engage-inner">
            ...
            <div className="py-plans-grid">
              <article className="py-pcard">
                <span className="py-pcard-badge">One-time</span>
                <span className="py-pcard-name">Fixed-Price Project</span>
                <p className="py-pcard-blurb">Defined scope &amp; budget — ideal for clearly-scoped builds with a fixed timeline and deliverables.</p>
                <ul className="py-pcard-features">
                ...
                <a href="#contact" className="py-pcard-cta">Get a Quote</a>
              </article>
              <article className="py-pcard">
                <span className="py-pcard-badge">Flexible</span>
                <span className="py-pcard-name">Time &amp; Materials</span>
                <p className="py-pcard-blurb">Scale with evolving requirements — pay for what's delivered, adjust scope any sprint.</p>
                <ul className="py-pcard-features">
                  <li>Shared specialist team</li>
              ...
              <article className="py-pcard py-pcard--feat">
                <span className="py-pcard-pop">✦ Most Popular</span>
                <span className="py-pcard-name">Dedicated Team</span>
                <p className="py-pcard-blurb">Your extended team — specialists embedded in your workflow, committed to your long-term growth.</p>
                <ul className="py-pcard-features">
                ...
                <a href="#contact" className="py-pcard-cta py-pcard-cta--feat">Get Started</a>
              </article>
              <article className="py-pcard">
                <span className="py-pcard-badge">Cost-efficient</span>
                <span className="py-pcard-name">Offshore Model</span>
                <p className="py-pcard-blurb">Maximum ROI with our expert offshore team — senior-level quality at budget-friendly rates.</p>
                <ul className="py-pcard-features">
          ...
          <div className="py-contact-container">
            ...
            <div className="py-contact-right">
              <div className="py-form-box">
                <h3>Contact Us</h3>
                <form className="py-contact-form" onSubmit={_sfSubmit}>
                  <div className="py-form-row">
                    <div className="py-form-group"><label>Full Name*</label><input name="sf-name" type="text" placeholder="Full Name*" required /></div>
                    <div className="py-form-group"><label>Business Email*</label><input type="email" name="sf-email" placeholder="Business Email Address*" required /></div>
                  </div>
                  <div className="py-form-row">
                    ...
                    <div className="py-form-group"><label>Organization*</label><input name="sf-company" type="text" placeholder="Organization / Institution*" required /></div>
                  </div>
                  <div className="py-form-group full"><label>Message*</label><textarea name="sf-message" placeholder="Describe your Python project requirements..." rows={6} required /></div>
