...
export default function HireNodeJSDeveloper() {
  ...
  const [c1, s1] = useCountUp(65);
  const [c2, s2] = useCountUp(280);
  ...
  useEffect(() => {
    ...
    const o5 = new IntersectionObserver(([e]) => { if (e.isIntersecting) { setStV(true); s1(65); s2(280); s3(49); s4(7); o5.disconnect(); } }, { threshold: 0.2 });
  ...
  return (
    <>
      <Head>
        ...
        <style>{`
          .hnjs-hero{background:linear-gradient(135deg,${ACCENT} 0%,#0a2800 60%,#143800 100%);color:#fff;padding:100px 20px 80px;text-align:center}
      ...
      <section className="hnjs-stats" ref={stGr}>
        <div className="hnjs-stats-grid">
          <div><div className="hnjs-stat-val">{stV ? c1 : 0}+</div><div className="hnjs-stat-label">Node.js Developers</div></div>
          <div><div className="hnjs-stat-val">{stV ? c2 : 0}+</div><div className="hnjs-stat-label">APIs & Services Built</div></div>
          <div><div className="hnjs-stat-val">4.{stV ? c3 : 0}/5</div><div className="hnjs-stat-label">Client Satisfaction</div></div>
          <div><div className="hnjs-stat-val">{stV ? c4 : 0} Days</div><div className="hnjs-stat-label">Avg Onboarding</div></div>
        </div>
      </section>
