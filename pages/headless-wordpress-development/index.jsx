    ...
    try {
      ...
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          source: 'Headless Wordpress Development', consent: true, recaptchaToken: token,
        }),
      });
    ...
    <>
      <Head>
        ...
        <style>{`
          ...
          /* Orbs */
          .hwp-aurora { position:absolute; inset:-15%; z-index:0; pointer-events:none; filter:blur(70px) saturate(150%); animation:hwp-aurora-drift 20s ease-in-out infinite alternate; }
          .hwp-aurora-b1 { position:absolute; left:20%; top:30%; width:65%; height:65%; border-radius:50%; background:radial-gradient(circle at center,rgba(15,52,96,0.28) 0%,transparent 70%); transform:translate(-50%,-50%); }
          ...
          .hwp-plans-grid { display:grid;grid-template-columns:repeat(4,1fr);gap:20px;align-items:start; }
          .hwp-pcard { display:flex;flex-direction:column;height:100%;padding:30px 26px;border-radius:20px;border:1.5px solid rgba(15,52,96,0.12);background:rgba(255,255,255,0.80);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);position:relative; }
          .hwp-pcard--feat { border-color:rgba(217,119,6,0.40);background:linear-gradient(180deg,rgba(254,243,199,0.35) 0%,rgba(255,255,255,0.90) 100%);box-shadow:0 0 60px rgba(217,119,6,0.12),0 8px 40px rgba(15,52,96,0.08); }
          .hwp-pcard-pop { position:absolute;top:-13px;left:50%;transform:translateX(-50%);padding:4px 14px;border-radius:100px;background:linear-gradient(90deg,#0F3460,#1a5276 50%,#D97706);color:#fff;font-size:10px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;white-space:nowrap;box-shadow:0 4px 16px rgba(217,119,6,0.28); }
          .hwp-pcard-badge { display:inline-block;font-size:10px;font-weight:700;letter-spacing:0.10em;text-transform:uppercase;color:#6B7280;background:rgba(15,52,96,0.06);padding:4px 12px;border-radius:100px;margin-bottom:14px; }
          .hwp-pcard-name { display:block;font-size:17px;font-weight:800;color:#0F3460;margin-bottom:10px;line-height:1.3; }
          .hwp-pcard--feat .hwp-pcard-name { color:#b45309; }
          .hwp-pcard-blurb { font-size:13px;color:#4A6080;line-height:1.7;margin:0 0 20px; }
          .hwp-pcard-features { list-style:none;padding:0;margin:0 0 24px;flex:1; }
          .hwp-pcard-features li { padding:8px 0;border-top:1px dashed rgba(15,52,96,0.10);font-size:13.5px;color:#374151;display:flex;align-items:flex-start;gap:8px; }
          .hwp-pcard-features li::before { content:'✓';color:#16a34a;font-weight:700;flex-shrink:0; }
          .hwp-pcard-cta { display:block;padding:12px 20px;background:rgba(15,52,96,0.07);border:1.5px solid rgba(15,52,96,0.18);border-radius:50px;color:#0F3460;font-size:13.5px;font-weight:700;text-decoration:none;text-align:center;transition:all 0.2s;margin-top:auto; }
          .hwp-pcard-cta:hover { background:rgba(15,52,96,0.12);transform:translateY(-1px);text-decoration:none; }
          .hwp-pcard-cta--feat { background:linear-gradient(135deg,#0F3460,#1a5276);border-color:transparent;color:#fff;box-shadow:0 4px 20px rgba(15,52,96,0.28); }
          .hwp-pcard-cta--feat:hover { background:linear-gradient(135deg,#0a2444,#0F3460);box-shadow:0 6px 28px rgba(15,52,96,0.38);color:#fff; }
          @media(max-width:1024px){ .hwp-plans-grid { grid-template-columns:repeat(2,1fr); } }
          @media(max-width:600px){ .hwp-plans-grid { grid-template-columns:1fr; } }
      ...
      <div className="hwp-page">
        <div className="hwp-aurora" aria-hidden="true">
          <div className="hwp-aurora-b1" />
          <div className="hwp-aurora-b2" />
          <div className="hwp-aurora-b3" />
        </div>
        ...
        <section className="hwp-engage-section" id="engagement">
          <div className="hwp-engage-inner">
            ...
            <div className="hwp-plans-grid">
              <article className="hwp-pcard">
                <span className="hwp-pcard-badge">One-time</span>
                <span className="hwp-pcard-name">Fixed-Price Project</span>
                <p className="hwp-pcard-blurb">Defined scope &amp; budget — ideal for clearly-scoped builds with a fixed timeline and deliverables.</p>
                <ul className="hwp-pcard-features">
                ...
                <a href="#contact" className="hwp-pcard-cta">Get a Quote</a>
              </article>
              <article className="hwp-pcard">
                <span className="hwp-pcard-badge">Flexible</span>
                <span className="hwp-pcard-name">Time &amp; Materials</span>
                <p className="hwp-pcard-blurb">Scale with evolving requirements — pay for what's delivered, adjust scope any sprint.</p>
                <ul className="hwp-pcard-features">
                  <li>Shared specialist team</li>
              ...
              <article className="hwp-pcard hwp-pcard--feat">
                <span className="hwp-pcard-pop">✦ Most Popular</span>
                <span className="hwp-pcard-name">Dedicated Team</span>
                <p className="hwp-pcard-blurb">Your extended team — specialists embedded in your workflow, committed to your long-term growth.</p>
                <ul className="hwp-pcard-features">
                ...
                <a href="#contact" className="hwp-pcard-cta hwp-pcard-cta--feat">Get Started</a>
              </article>
              <article className="hwp-pcard">
                <span className="hwp-pcard-badge">Cost-efficient</span>
                <span className="hwp-pcard-name">Offshore Model</span>
                <p className="hwp-pcard-blurb">Maximum ROI with our expert offshore team — senior-level quality at budget-friendly rates.</p>
                <ul className="hwp-pcard-features">
          ...
          <div className="hwp-contact-container">
            ...
            <div className="hwp-contact-right">
              <div className="hwp-form-box">
                <h3>Contact Us</h3>
                <form className="hwp-contact-form" onSubmit={_sfSubmit}>
                  <div className="hwp-form-row">
                    <div className="hwp-form-group"><label>Full Name*</label><input name="sf-name" type="text" placeholder="Full Name*" required /></div>
                    <div className="hwp-form-group"><label>Business Email*</label><input type="email" name="sf-email" placeholder="Business Email Address*" required /></div>
                  </div>
                  <div className="hwp-form-row">
