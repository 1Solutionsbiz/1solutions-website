    ...
    try {
      ...
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          source: 'Ev Software Development Company', consent: true, recaptchaToken: token,
        }),
      });
  ...
  return (
    <>
      ...
      <div className="ev-page">
        ...
        <div className="ev-fb"><h3>Tell Us About Your EV Project</h3><form className="ev-form" onSubmit={_sfSubmit}><div className="ev-fr"><div className="ev-fg"><label>Full Name *</label><input name="sf-name" type="text" placeholder="Your name" required /></div><div className="ev-fg"><label>Work Email *</label><input type="email" name="sf-email" placeholder="you@company.com" required /></div></div><div className="ev-fr"><div className="ev-fg"><label>Company</label><input name="sf-name" type="text" placeholder="Company name" /></div><div className="ev-fg"><label>Phone / WhatsApp *</label><input type="tel" name="sf-phone" placeholder="+1 555 000 0000" required /></div></div><div className="ev-fg full"><label>Type of EV Platform *</label><select required><option value="">Select...</option><option>Charge Station Management System (CSMS)</option><option>EV Driver App</option><option>Fleet Electrification Platform</option><option>Energy Management System (EMS)</option><option>Charge Network Operator Back-Office</option><option>Battery Health & Analytics</option><option>V2G / V2B Integration</option><option>EV Telematics Platform</option><option>OCPI Roaming Integration</option><option>EV Analytics Dashboard</option><option>Other</option></select></div><div className="ev-fg full"><label>Project Description *</label><textarea name="sf-message" rows={4} placeholder="Describe your EV software project - type of platform, number of charge points or vehicles, current systems, OCPP version required, and go-live timeline..." required /></div><div className="ev-co"><input type="checkbox" required /><label>I agree to the <Link href="/privacy-policy">Privacy Policy</Link>. Details are treated confidentially.</label></div><button type="submit" className="ev-sub">Get an EV Software Proposal →</button>
