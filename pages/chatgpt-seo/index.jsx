...
const STATS = [
  ['Brands Optimised for ChatGPT','150+'],
  ['Avg Citation Increase','3x'],
...
function Stat({ label, val, started }) {
  ...
  return (
    <div className="cgpt-stat-col">
      <div className="cgpt-stat-val">{started ? n + sfx : val}</div>
      <div className="cgpt-stat-lbl">{label}</div>
    </div>
  );
}
  ...
  return (
    <>
      <Head>
        ...
        <style>{`
          .cgpt-hero{background:linear-gradient(135deg,#0F3460 0%,#1a1a5e 40%,#0F3460 100%);padding:80px 40px 0;position:relative;overflow:hidden}
          .cgpt-hero-orb{position:absolute;width:600px;height:600px;border-radius:50%;background:radial-gradient(circle,rgba(217,119,6,0.18) 0%,transparent 65%);top:-180px;right:-100px;pointer-events:none;filter:blur(40px)}
          .cgpt-hero-inner{max-width:1280px;margin:0 auto;padding-bottom:60px}
          .cgpt-eyebrow{display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.10);border:1px solid rgba(255,255,255,0.20);border-radius:100px;padding:5px 14px;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:rgba(255,255,255,0.85);margin-bottom:24px}
          .cgpt-h1{font-size:clamp(2rem,4vw,3.2rem);font-weight:900;line-height:1.1;letter-spacing:-1px;color:#fff;margin:0 0 18px}
          .cgpt-h1 span{background:linear-gradient(90deg,#FCD34D,#F97316);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
          .cgpt-sub{font-size:1.05rem;color:rgba(255,255,255,0.75);line-height:1.75;margin:0 0 34px;max-width:600px}
          .cgpt-btns{display:flex;gap:14px;flex-wrap:wrap}
          ...
          .cgpt-btn-g{display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.10);backdrop-filter:blur(12px);border:1.5px solid rgba(255,255,255,0.25);color:#fff;padding:13px 28px;border-radius:50px;font-weight:700;font-size:0.93rem;text-decoration:none;transition:all .25s}
          .cgpt-btn-g:hover{background:rgba(255,255,255,0.18);transform:translateY(-2px)}
          .cgpt-stats{background:rgba(255,255,255,0.06);border-top:1px solid rgba(255,255,255,0.10);padding:20px 40px}
          .cgpt-stats-inner{max-width:1280px;margin:0 auto;display:grid;grid-template-columns:repeat(4,1fr)}
          .cgpt-stat-col{text-align:center;padding:16px;border-right:1px solid rgba(255,255,255,0.10)}
          .cgpt-stat-col:last-child{border-right:none}
          .cgpt-stat-val{font-size:1.9rem;font-weight:900;color:#fff;letter-spacing:-1px}
          .cgpt-stat-lbl{font-size:11px;color:rgba(255,255,255,0.50);font-weight:500;text-transform:uppercase;letter-spacing:1px;margin-top:4px}
          ...
          @media(max-width:768px){
            .cgpt-hero,.cgpt-svc,.cgpt-faq,.cgpt-cta,.cgpt-related{padding:60px 20px}
            .cgpt-stats,.cgpt-stats-inner{padding:16px 20px}
            .cgpt-stats-inner{grid-template-columns:repeat(2,1fr)}
            .cgpt-grid{grid-template-columns:1fr}
          }
        `}</style>
      </Head>

      <section className="cgpt-hero">
        <div className="cgpt-hero-orb" />
        <div className="cgpt-hero-inner">
          <span className="cgpt-eyebrow">
            <span style={{width:6,height:6,borderRadius:'50%',background:'#D97706',display:'inline-block'}} />
            1Solutions AI Practice
          </span>
          <h1 className="cgpt-h1">ChatGPT SEO Services<br /><span>Get Your Brand Cited in ChatGPT</span></h1>
          <p className="cgpt-sub">Over 200 million people use ChatGPT weekly. When they ask about your industry, your competitors, or your services — is your brand part of the answer? We build the authority signals that make ChatGPT cite your brand.</p>
          <div className="cgpt-btns">
            <Link href="#contact" className="cgpt-btn-p">Get a Free Consultation →</Link>
            <Link href="#services" className="cgpt-btn-g">View Services</Link>
          </div>
        </div>
        <div className="cgpt-stats" ref={statsRef}>
          <div className="cgpt-stats-inner">
