...
const FAQS = [
  ...
  { q:'Do you work with clients in the US, Canada, and Australia remotely?', a:'Yes - 100% of our client work is delivered remotely. We have been working with clients across the US, Canada, and Australia since 2010. We schedule meetings in your time zone, use Slack, Notion, and Loom for transparent collaboration, and provide full project visibility at every stage.' },
    ...
    try {
      ...
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          source: 'Woocommerce Development Company', consent: true, recaptchaToken: token,
        }),
      });
    ...
    <>
      <Head>
        ...
        <style>{`
          ...
          /* Orbs */
          .woo-aurora { position:absolute; inset:-15%; z-index:0; pointer-events:none; filter:blur(70px) saturate(150%); animation:woo-aurora-drift 20s ease-in-out infinite alternate; }
          .woo-aurora-b1 { position:absolute; left:20%; top:30%; width:65%; height:65%; border-radius:50%; background:radial-gradient(circle at center,rgba(15,52,96,0.28) 0%,transparent 70%); transform:translate(-50%,-50%); }
          ...
          /* Engagement Table */
          .woo-engage-section { background:#f8fafd;border-top:1px solid rgba(15,52,96,0.08);border-bottom:1px solid rgba(15,52,96,0.08);padding:80px 40px;position:relative;z-index:1; }
          .woo-engage-inner { max-width:1280px;margin:0 auto; }
          .woo-engage-header { text-align:center;margin-bottom:52px; }
          .woo-engage-title { font-size:48px;font-weight:900;line-height:1.15;letter-spacing:-1px;background:linear-gradient(90deg,#0F3460 0%,#D97706 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:transparent;margin:0 0 14px; }
          .woo-engage-desc { font-size:15px;color:#3A507A;line-height:1.7;max-width:640px;margin:0 auto; }
          .woo-plans-grid { display:grid;grid-template-columns:repeat(4,1fr);gap:20px;align-items:start; }
          .woo-pcard { display:flex;flex-direction:column;height:100%;padding:30px 26px;border-radius:20px;border:1.5px solid rgba(15,52,96,0.12);background:rgba(255,255,255,0.80);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);position:relative; }
          .woo-pcard--feat { border-color:rgba(217,119,6,0.40);background:linear-gradient(180deg,rgba(254,243,199,0.35) 0%,rgba(255,255,255,0.90) 100%);box-shadow:0 0 60px rgba(217,119,6,0.12),0 8px 40px rgba(15,52,96,0.08); }
          .woo-pcard-pop { position:absolute;top:-13px;left:50%;transform:translateX(-50%);padding:4px 14px;border-radius:100px;background:linear-gradient(90deg,#0F3460,#1a5276 50%,#D97706);color:#fff;font-size:10px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;white-space:nowrap;box-shadow:0 4px 16px rgba(217,119,6,0.28); }
          .woo-pcard-badge { display:inline-block;font-size:10px;font-weight:700;letter-spacing:0.10em;text-transform:uppercase;color:#6B7280;background:rgba(15,52,96,0.06);padding:4px 12px;border-radius:100px;margin-bottom:14px; }
          .woo-pcard-name { display:block;font-size:17px;font-weight:800;color:#0F3460;margin-bottom:10px;line-height:1.3; }
          .woo-pcard--feat .woo-pcard-name { color:#b45309; }
          .woo-pcard-blurb { font-size:13px;color:#4A6080;line-height:1.7;margin:0 0 20px; }
          .woo-pcard-features { list-style:none;padding:0;margin:0 0 24px;flex:1; }
          .woo-pcard-features li { padding:8px 0;border-top:1px dashed rgba(15,52,96,0.10);font-size:13.5px;color:#374151;display:flex;align-items:flex-start;gap:8px; }
          .woo-pcard-features li::before { content:'✓';color:#16a34a;font-weight:700;flex-shrink:0; }
          .woo-pcard-cta { display:block;padding:12px 20px;background:rgba(15,52,96,0.07);border:1.5px solid rgba(15,52,96,0.18);border-radius:50px;color:#0F3460;font-size:13.5px;font-weight:700;text-decoration:none;text-align:center;transition:all 0.2s;margin-top:auto; }
          .woo-pcard-cta:hover { background:rgba(15,52,96,0.12);transform:translateY(-1px);text-decoration:none; }
          .woo-pcard-cta--feat { background:linear-gradient(135deg,#0F3460,#1a5276);border-color:transparent;color:#fff;box-shadow:0 4px 20px rgba(15,52,96,0.28); }
          .woo-pcard-cta--feat:hover { background:linear-gradient(135deg,#0a2444,#0F3460);box-shadow:0 6px 28px rgba(15,52,96,0.38);color:#fff; }
          @media(max-width:1024px){ .woo-plans-grid { grid-template-columns:repeat(2,1fr); } }
          @media(max-width:600px){ .woo-plans-grid { grid-template-columns:1fr; } }
          ...
          .woo-faq-heading { font-size:48px;font-weight:900;line-height:1.15;letter-spacing:-1px;background:linear-gradient(135deg,#4f46e5,#7c3aed,#a855f7,#ec4899,#3b82f6,#06b6d4,#4f46e5);background-size:300% 300%;animation:aurora-text 6s ease infinite;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:transparent;margin:0 0 36px; }
          ...
          .woo-related-title { font-size:48px;font-weight:900;line-height:1.15;letter-spacing:-1px;background:linear-gradient(135deg,#4f46e5,#7c3aed,#a855f7,#ec4899,#3b82f6,#06b6d4,#4f46e5);background-size:300% 300%;animation:aurora-text 6s ease infinite;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;color:transparent;margin:0 0 16px; }
      ...
      <div className="woo-page">
        <div className="woo-aurora" aria-hidden="true">
          <div className="woo-aurora-b1" />
          <div className="woo-aurora-b2" />
          <div className="woo-aurora-b3" />
        </div>
        ...
        {/* ── ENGAGEMENT MODELS ── */}
        <section className="woo-engage-section" id="engagement">
          <div className="woo-engage-inner">
            <div className="woo-engage-header">
              <span className="woo-section-eyebrow">How We Engage</span>
              <h2 className="woo-engage-title">Flexible Engagement Models</h2>
              <p className="woo-engage-desc">Pick the model that fits your project, team, and budget — every plan includes a free discovery call and NDA on request.</p>
            </div>

            <div className="woo-plans-grid">
              <article className="woo-pcard">
                <span className="woo-pcard-badge">One-time</span>
                <span className="woo-pcard-name">Fixed-Price Project</span>
                <p className="woo-pcard-blurb">Defined scope &amp; budget — ideal for clearly-scoped builds with a fixed timeline and deliverables.</p>
                <ul className="woo-pcard-features">
                ...
                <a href="#contact" className="woo-pcard-cta">Get a Quote</a>
              </article>
              <article className="woo-pcard">
                <span className="woo-pcard-badge">Flexible</span>
                <span className="woo-pcard-name">Time &amp; Materials</span>
                <p className="woo-pcard-blurb">Scale with evolving requirements — pay for what's delivered, adjust scope any sprint.</p>
                <ul className="woo-pcard-features">
                  <li>Shared specialist team</li>
              ...
              <article className="woo-pcard woo-pcard--feat">
                <span className="woo-pcard-pop">✦ Most Popular</span>
                <span className="woo-pcard-name">Dedicated Team</span>
                <p className="woo-pcard-blurb">Your extended team — specialists embedded in your workflow, committed to your long-term growth.</p>
                <ul className="woo-pcard-features">
                ...
                <a href="#contact" className="woo-pcard-cta woo-pcard-cta--feat">Get Started</a>
              </article>
              <article className="woo-pcard">
                <span className="woo-pcard-badge">Cost-efficient</span>
                <span className="woo-pcard-name">Offshore Model</span>
                <p className="woo-pcard-blurb">Maximum ROI with our expert offshore team — senior-level quality at budget-friendly rates.</p>
                <ul className="woo-pcard-features">
          ...
          <div className="woo-contact-container">
            ...
            <div className="woo-contact-right">
              <div className="woo-form-box">
                <h3>Contact Us</h3>
                <form className="woo-contact-form" onSubmit={_sfSubmit}>
                  <div className="woo-form-row">
                    <div className="woo-form-group"><label>Full Name*</label><input name="sf-name" type="text" placeholder="Full Name*" required /></div>
                    <div className="woo-form-group"><label>Business Email*</label><input type="email" name="sf-email" placeholder="Business Email Address*" required /></div>
                  </div>
                  <div className="woo-form-row">
                    ...
                    <div className="woo-form-group"><label>Organization*</label><input name="sf-company" type="text" placeholder="Organization / Store Name*" required /></div>
                  </div>
                  <div className="woo-form-group full"><label>Message*</label><textarea name="sf-message" placeholder="Tell us about your WooCommerce project..." rows={6} required /></div>
