    ...
    try {
      ...
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          ...
          source: 'Social Media App Development Company', consent: true, recaptchaToken: token,
        }),
      });
    ...
    <>
      <Head>
        ...
        <style>{`
          ...
          .socm-ri h2{font-size:30px;font-weight:900;background:linear-gradient(135deg,#4f46e5,#7c3aed,#a855f7,#ec4899,#3b82f6,#06b6d4,#4f46e5);background-size:300% 300%;animation:aurora-text 6s ease infinite;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin:10px 0 10px}
      ...
      <div className="socm-page">
        ...
        <div className="socm-fb"><h3>Tell Us About Your Social Platform</h3><form className="socm-form" onSubmit={_sfSubmit}><div className="socm-fr"><div className="socm-fg"><label>Full Name *</label><input name="sf-name" type="text" placeholder="Your name" required /></div><div className="socm-fg"><label>Work Email *</label><input type="email" name="sf-email" placeholder="you@company.com" required /></div></div><div className="socm-fr"><div className="socm-fg"><label>Company</label><input name="sf-name" type="text" placeholder="Company name" /></div><div className="socm-fg"><label>Phone / WhatsApp</label><input type="tel" name="sf-phone" placeholder="+1 555 000 0000" /></div></div><div className="socm-fg full"><label>Type of Social Platform *</label><select required><option value="">Select...</option><option>Niche Social Network</option><option>Short-Form Video App (Reels/TikTok-style)</option><option>Creator Monetisation Platform</option><option>Fan Engagement & Community App</option><option>Social Commerce Platform</option><option>Real-Time Messaging / Group Chat</option><option>Content Moderation System</option><option>Live Streaming & Rooms</option><option>Social Graph & Recommendation Engine</option><option>Social Analytics Dashboard</option><option>Other</option></select></div><div className="socm-fg full"><label>Project Description *</label><textarea name="sf-message" rows={4} placeholder="Describe your social platform - target community or audience, core features, monetisation model, expected user scale, and go-live timeline..." required /></div><div className="socm-co"><input type="checkbox" required /><label>I agree to the <Link href="/privacy-policy/">Privacy Policy</Link>. Details are treated confidentially.</label></div><button type="submit" className="socm-sub">Get a Social Platform Proposal →</button>
        ...
        <section className="socm-rel"><div className="socm-ri"><span className="socm-sey">Related Services</span><h2>Related Industry & Technology Services</h2><hr /><div className="socm-rts">{[['/entertainment-software-development/','Entertainment Software','socm-ra'],['/mobile-app-development/','Mobile App Development','socm-rb'],['/retail-ecommerce-software-development/','Retail & eCommerce','socm-rc'],['/saas-application-development-company/','SaaS Development','socm-rd'],['/it-outsourcing-services/','IT Outsourcing','socm-ra'],['/offshore-development-company/','Offshore Development','socm-rb'],['/ai-solutions/','AI/ML Development','socm-rc'],['/web-development/','Web Development','socm-rd']].map(([hr, lb, cl]) => (<Link key={hr} href={hr} className={`socm-rt ${cl}`}>{lb}</Link>))}</div></div></section>
      </div>
    </>
  );
}
