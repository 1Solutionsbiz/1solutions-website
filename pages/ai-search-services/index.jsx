...
const STATS = [
  ['AI Platforms Optimised','12+'],
  ['Brands Cited in AI','300+'],
...
function Stat({ label, val, started }) {
  ...
  return (
    <div className="ais-stat-col">
      <div className="ais-stat-val">{started ? n + sfx : val}</div>
      <div className="ais-stat-lbl">{label}</div>
    </div>
  );
}
  ...
  return (
    <>
      <Head>
        ...
        <style>{`
          .ais-hero{background:linear-gradient(135deg,#0F3460 0%,#1a1a5e 40%,#0F3460 100%);padding:80px 40px 0;position:relative;overflow:hidden}
          .ais-hero-orb{position:absolute;width:600px;height:600px;border-radius:50%;background:radial-gradient(circle,rgba(217,119,6,0.18) 0%,transparent 65%);top:-180px;right:-100px;pointer-events:none;filter:blur(40px)}
          .ais-hero-inner{max-width:1280px;margin:0 auto;padding-bottom:60px}
          .ais-eyebrow{display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.10);border:1px solid rgba(255,255,255,0.20);border-radius:100px;padding:5px 14px;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:rgba(255,255,255,0.85);margin-bottom:24px}
          .ais-h1{font-size:clamp(2rem,4vw,3.2rem);font-weight:900;line-height:1.1;letter-spacing:-1px;color:#fff;margin:0 0 18px}
          .ais-h1 span{background:linear-gradient(90deg,#FCD34D,#F97316);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
          .ais-sub{font-size:1.05rem;color:rgba(255,255,255,0.75);line-height:1.75;margin:0 0 34px;max-width:600px}
          .ais-btns{display:flex;gap:14px;flex-wrap:wrap}
          ...
          .ais-btn-g{display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,0.10);backdrop-filter:blur(12px);border:1.5px solid rgba(255,255,255,0.25);color:#fff;padding:13px 28px;border-radius:50px;font-weight:700;font-size:0.93rem;text-decoration:none;transition:all .25s}
          .ais-btn-g:hover{background:rgba(255,255,255,0.18);transform:translateY(-2px)}
          .ais-stats{background:rgba(255,255,255,0.06);border-top:1px solid rgba(255,255,255,0.10);padding:20px 40px}
          .ais-stats-inner{max-width:1280px;margin:0 auto;display:grid;grid-template-columns:repeat(4,1fr)}
          .ais-stat-col{text-align:center;padding:16px;border-right:1px solid rgba(255,255,255,0.10)}
          .ais-stat-col:last-child{border-right:none}
          .ais-stat-val{font-size:1.9rem;font-weight:900;color:#fff;letter-spacing:-1px}
          .ais-stat-lbl{font-size:11px;color:rgba(255,255,255,0.50);font-weight:500;text-transform:uppercase;letter-spacing:1px;margin-top:4px}
          ...
          @media(max-width:768px){
            .ais-hero,.ais-svc,.ais-faq,.ais-cta,.ais-related{padding:60px 20px}
            .ais-stats,.ais-stats-inner{padding:16px 20px}
            .ais-stats-inner{grid-template-columns:repeat(2,1fr)}
      ...
      <section className="ais-hero">
        <div className="ais-hero-orb" />
        <div className="ais-hero-inner">
          <span className="ais-eyebrow">
            <span style={{width:6,height:6,borderRadius:'50%',background:'#D97706',display:'inline-block'}} />
            1Solutions AI Practice
          </span>
          <h1 className="ais-h1">AI Search Services<br /><span>Be Found Everywhere AI Answers</span></h1>
          <p className="ais-sub">People are searching in ChatGPT, Perplexity, Google AI Overviews, and Meta AI — not just Google. We help your brand get cited, recommended, and found across every AI-powered search surface.</p>
          <div className="ais-btns">
            <Link href="#contact" className="ais-btn-p">Get a Free Consultation →</Link>
            <Link href="#services" className="ais-btn-g">View Services</Link>
          </div>
        </div>
        <div className="ais-stats" ref={statsRef}>
          <div className="ais-stats-inner">
